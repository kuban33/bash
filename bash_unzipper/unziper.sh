...
# 1.1 - 2018-08-29 SARA
# - generic purpose unziper
# - refactored, changed wtfPOWER namings to generic e.g Processed to ok, NotProcessed to ko, etc.
# 1.0 - 2018-06-22 SARA
# - unzips zip files in provided folder, puts the to outdir, optionally keep original files and failed in sep dir
# ------------------------------------------------------------------------------------------------------------------
#config
...
then
  ...
  echo "USAGE: $0 inputdir zipsuffix [outputdir [kodir [okdir]]]"
  echo "PARAMETERS:"
  echo "  inputdir = path to input dir containing zip files"
  echo "  zipsuffix = expected zip file suffix for which will be searched"
  echo "  outputdir = [optional] if specified unziped output files moved here, otherwise moved to inputdir"
  ...
  echo "  $0 in/ .zip"
  echo "  $0 in/ .zip out/"
  echo "  $0 in/ .zip out/ ko/"
  echo "  $0 in/ .zip out/ ko/ ok/"
  exit 1
...
do
  ...
  #unzip test
  unzip -t "${VAR_FILE_work_in}"
  if (( $? == 0 ))
  then
    #unzip overwrite, without dirs
    unzip -oj "${VAR_FILE_work_in}" -d "${VAR_DIR_work}"
    if (( $? == 0 ))
    then
      if [[ -n "${VAR_DIR_ok}" ]]
      then
